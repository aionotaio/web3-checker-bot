...
from shared.vars import TICKERS, FORMATTED_NAMES, CLAIM_LINKS, FULL_RESULTS_TEMP_CSV, FULL_RESULTS_CSV, ELIGIBLE_RESULTS_TEMP_CSV, ELIGIBLE_RESULTS_CSV
...
async def handle_download_full_csv(callback: types.CallbackQuery, state: FSMContext) -> None:
    ...
    inline_keyboard = [[types.InlineKeyboardButton(text="❌ Delete file", callback_data="delete_file")]]
...
async def handle_download_eligible_csv(callback: types.CallbackQuery, state: FSMContext) -> None:
    ...
    input_file = types.FSInputFile(
        ELIGIBLE_RESULTS_TEMP_CSV, filename=ELIGIBLE_RESULTS_CSV
    )
