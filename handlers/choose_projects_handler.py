...
from src.vars import FORMATTED_NAMES
...
async def choose_projects(callback: types.CallbackQuery, state: FSMContext):
    ...
    inline_keyboard.append([types.InlineKeyboardButton(text="✅ Выбрать все", callback_data="choose_all")])
    inline_keyboard.append([types.InlineKeyboardButton(text="❌ Убрать все", callback_data="deselect_all")])
    ...
    await msg.edit_text("❓ Выберите проекты, для которых хотите проверить аллокацию", reply_markup=builder)
